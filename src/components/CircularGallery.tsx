...
function createTextTexture(
  gl: GL,
  text: string,
  font: string = "bold 24px monospace",
  color: string = "black",
...
class Title {
  ...
  createMesh() {
    ...
    const textHeightScaled = this.plane.scale.y * 0.12; // Thu nhỏ chữ lại một chút cho tinh tế
    const textWidthScaled = textHeightScaled * aspect;
    this.mesh.scale.set(textWidthScaled, textHeightScaled, 1);
    this.mesh.position.y =
      -this.plane.scale.y * 0.5 - textHeightScaled * 0.5 - 0.05; // Đẩy chữ xuống thấp tránh dính sát ảnh
...
class Media {
  ...
  update(
    ...
    this.isBefore = this.plane.position.x + planeOffset < -viewportOffset * 2.0;
    this.isAfter = this.plane.position.x - planeOffset > viewportOffset * 2.0;
  ...
  onResize({
    ...
    // PHÓNG TO ẢNH: Đẩy thông số tỷ lệ mặt phẳng WebGL lên (Cao 650, Rộng 500) để ảnh trông to, rõ nét hơn
    this.plane.scale.y =
    ...
    this.padding = 1.2; // Giãn rộng khoảng cách giữa các khối ảnh lớn
...
class App {
  ...
  constructor(
    container: HTMLElement,
    {
      ...
      textColor = "#E1E0CC",
      borderRadius = 0.04,
      font = "normal 24px Figtree",
      scrollSpeed = 1.2, // GIẢM TỐC: Hạ tốc độ cuộn tổng thể xuống từ 2 -> 1.2
      scrollEase = 0.08, // ĐẦM HƠN: Tăng độ mượt hãm phanh giúp dải ảnh dừng chắc chắn, không trượt tự do quá đà
    }: AppConfig,
  ) {
  ...
  createCamera() {
    ...
    this.camera.fov = 45;
    this.camera.position.z = 12; // Điều chỉnh khoảng cách camera cân bằng với kích thước ảnh lớn mới
  }
  ...
  onWheel(e: Event) {
    ...
    // ĐIỀU CHỈNH CUỘN: Tối ưu lại bước nhảy của con lăn chuột cho mượt mà, vừa vặn
    this.scroll.target +=
      (delta > 0 ? this.scrollSpeed : -this.scrollSpeed) * 0.12;
...
export default function CircularGallery({
  items,
  bend = 1.0, // Độ cong vừa vặn với kích thước ảnh phóng to
  textColor = "#E1E0CC",
  borderRadius = 0.04,
  font = "normal 24px Figtree",
  scrollSpeed = 1.2,
